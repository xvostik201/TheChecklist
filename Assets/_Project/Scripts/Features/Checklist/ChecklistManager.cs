...
namespace TheChecklist.Features.Checklist
{
    public class ChecklistManager : IInitializable, IDisposable, ITickable
    {
        ...
        private void OnStateChange(string elementID, bool newState)
        {
            ...
            {
                var step = _checklistSteps[i];
        
                if (step.TargetElementID == elementID)
                {
                    if (step.IsPersistent && newState != step.RequiredState)
                    {
                        Debug.Log($"<color=orange>Rollback!</color> Step {step.Description} state changed, but it was persistent.");
                        ResetToStep(i);
                        return; 
                    }
                }
        ...
        private void OnStateChange(string elementID, float newStateValue)
        {
            for (int i = 0; i < _currentStepIndex; i++)
            {
                ...
                if (step.TargetElementID == elementID)
                {
                    bool isValueWrong = Mathf.Abs(step.RequiredValue - newStateValue) > 0.05f;

                    if (step.IsPersistent && isValueWrong)
                    {
                        Debug.Log($"<color=orange>Rollback!</color> Normalized element {step.Description} changed value.");
                        ResetToStep(i);
                        return;
                    }
                }
